	...
	If GUICtrlRead($g_hChkAttackPlannerEnable) = $GUI_CHECKED Then
		...
		If GUICtrlRead($g_hChkAttackPlannerCloseAll) = $GUI_UNCHECKED Then
			...
			If GUICtrlRead($g_acmdRandomDelay) = $GUI_CHECKED Then 
				...
				GUICtrlSetState($g_hLabelDelayPauseIntervalUnit, $GUI_ENABLE)
			Else
				GUICtrlSetState($g_acmdRandomDelayMin, $GUI_DISABLE)
				GUICtrlSetState($g_hLabelDelayPauseIntervalAnd, $GUI_DISABLE)
				GUICtrlSetState($g_acmdRandomDelayMax, $GUI_DISABLE)
				GUICtrlSetState($g_hLabelDelayPauseIntervalUnit, $GUI_DISABLE)
			EndIf
...
Func chkAttackPlannerCloseCoC()
	...
	EndIf
	If GUICtrlRead($g_hChkAttackPlannerEnable) = $GUI_CHECKED And GUICtrlRead($g_hChkAttackPlannerCloseCoC) = $GUI_CHECKED And $g_bNotifyTGEnable = True Then
		GUICtrlSetState($g_hChkNotifyPauseTime, $GUI_ENABLE)
	Else
		GUICtrlSetState($g_hChkNotifyPauseTime, $GUI_DISABLE)
	EndIf
