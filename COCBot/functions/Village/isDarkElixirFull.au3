...
Func isDarkElixirFull($Reduction = False)
	...
	ElseIf $g_abFullStorage[$eLootDarkElixir] Then
		If Number($g_aiCurrentLoot[$eLootDarkElixir]) >= Number($g_aiResumeAttackLoot[$eLootDarkElixir]) And Not $Reduction Then
			SetLog("Dark Elixir Storage is relatively full: " & $g_aiCurrentLoot[$eLootDarkElixir], $COLOR_SUCCESS)
			$g_abFullStorage[$eLootDarkElixir] = True
		Else
			If Not $Reduction Then SetLog("Switching back to normal when Dark Elixir drops below " & $g_aiResumeAttackLoot[$eLootDarkElixir], $COLOR_SUCCESS)
