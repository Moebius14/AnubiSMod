	...
	If IsArray($aUpgradeButton) And UBound($aUpgradeButton, 1) = 2 Then
		...
		Select ;Ensure the right upgrade window is open!
			...
			Case _ColorCheck(_GetPixelColor(733, 167 + $g_iMidOffsetY, True), Hex(0xCD1218, 6), 20) ; Check if the Gear Up Building window is open
				...
				$g_avBuildingUpgrades[$inum][2] = Number(getCostsUpgrade(372, 474 + $g_iMidOffsetY)) ; Try to read white text.
				Local $g_ReadCorrect = StringRight($g_avBuildingUpgrades[$inum][2], 1)
				If $g_avBuildingUpgrades[$inum][2] = "" Or $g_avBuildingUpgrades[$inum][2] < 1000 Or $g_ReadCorrect <> 0 Then
					$g_avBuildingUpgrades[$inum][2] = Number(getCostsUpgrade1(372, 475 + $g_iMidOffsetY))
					$g_ReadCorrect = StringRight($g_avBuildingUpgrades[$inum][2], 1)
					If $g_avBuildingUpgrades[$inum][2] = "" Or $g_avBuildingUpgrades[$inum][2] < 1000 Or $g_ReadCorrect <> 0 Then
						$g_avBuildingUpgrades[$inum][2] = Number(getCostsUpgrade2(372, 475 + $g_iMidOffsetY))
						$g_ReadCorrect = StringRight($g_avBuildingUpgrades[$inum][2], 1)
					EndIf
					If $g_ReadCorrect <> 0 Then $g_avBuildingUpgrades[$inum][2] = ""
					If $g_avBuildingUpgrades[$inum][2] = "" Then $g_avBuildingUpgrades[$inum][2] = Number(getCostsUpgradeRed3(372, 474 + $g_iMidOffsetY)) ;read RED upgrade text
					$g_ReadCorrect = StringRight($g_avBuildingUpgrades[$inum][2], 1)
					If $g_avBuildingUpgrades[$inum][2] = "" Or $g_avBuildingUpgrades[$inum][2] < 1000 Or $g_ReadCorrect <> 0 Then
						$g_avBuildingUpgrades[$inum][2] = Number(getCostsUpgradeRed2(372, 475 + $g_iMidOffsetY))
						$g_ReadCorrect = StringRight($g_avBuildingUpgrades[$inum][2], 1)
						If $g_avBuildingUpgrades[$inum][2] = "" Or $g_avBuildingUpgrades[$inum][2] < 1000 Or $g_ReadCorrect <> 0 Then
							$g_avBuildingUpgrades[$inum][2] = Number(getCostsUpgradeRed(372, 475 + $g_iMidOffsetY))
							$g_ReadCorrect = StringRight($g_avBuildingUpgrades[$inum][2], 1)
						EndIf
					EndIf
					If $g_ReadCorrect <> 0 Then $g_avBuildingUpgrades[$inum][2] = ""
