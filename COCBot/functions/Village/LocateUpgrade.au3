...
Func UpgradeValue($inum, $bRepeat = False) ;function to find the value and type of the upgrade.
	...
	If $bRepeat Or $g_abUpgradeRepeatEnable[$inum] Then ; check for upgrade in process when continiously upgrading
		...
		Local $offColors[3][3] = [[0x0D0D0D, 39, 19], [0x914F31, 68, 34], [0xFFFFFF, 81, 0]] ; 2nd pixel black broken hammer, 3rd pixel lt brown handle, 4th pixel white edge of button
		Local $ButtonPixel = _MultiPixelSearch(284, 572, 580, 615, 1, 1, Hex(0x0E0E0D, 6), $offColors, 40) ; first black pixel on side of button
		SetDebugLog("Pixel Color #1: " & _GetPixelColor(390, 572, True) & ", #2: " & _GetPixelColor(429, 601, True) & ", #3: " & _GetPixelColor(458, 606, True) & ", #4: " & _GetPixelColor(471, 572, True), $COLOR_DEBUG)
	...
	If IsArray($aUpgradeButton) And UBound($aUpgradeButton, 1) = 2 Then
		...
		Select ;Ensure the right upgrade window is open!
			...
			Case _ColorCheck(_GetPixelColor(737, 134 + $g_iMidOffsetY, True), Hex(0xFF8D95, 6), 20) ; Check if the Gear Up Building window is open
				...
				If $g_avBuildingUpgrades[$inum][6] = "" Then $g_avBuildingUpgrades[$inum][6] = Number(getGearUpgradeTime(185, 392 + $g_iMidOffsetY)) ; Try to read yellow text (Discount).
				SetLog("Upgrade #" & $inum + 1 & " Time = " & $g_avBuildingUpgrades[$inum][6], $COLOR_INFO)
				If $g_avBuildingUpgrades[$inum][6] <> "" Then $g_avBuildingUpgrades[$inum][7] = "" ; Clear old upgrade end time
