		...
		If $g_bAttackCGPlannerEnable And $g_bAttackCGPlannerDayLimit And IsClanGamesWindow() Then
			...
			If $aiScoreLimit[0] < $aiScoreLimit[1] Then
				...
				ElseIf $g_bFirstStartCheckDone Then
					SetLog("It's A Brand New Day ! Reset Max Clan Games Challenges", $COLOR_FUCHSIA)
					SetLog("Max Clan Games Challenges Today : " & $iRandomAttackCGCountToday & "", $COLOR_FUCHSIA)
				EndIf
			EndIf
			CloseWindow()
			...
			If IsArray($aActiveEvent) And UBound($aActiveEvent) > 0 Then
				...
				If $g_bAttackCGPlannerEnable And $g_bAttackCGPlannerDayLimit And $g_bFirstStartCheckDone Then
					...
					If $remainingCGattacks = 0 Then
						SetLog("Running Last Challenge Of This Day", $COLOR_FUCHSIA)
...
Func PurgeEvent($bTest = False, $startFirst = True, $NoMistake = True, $iRow = 1, $NoOneFound = False)
	...
	If $g_bChkCCGDebugNoneFound And $NoMistake Then
		If $NoOneFound Then
			ChallengeNextPage(1, $iRow)
			If _Sleep(1000) Then Return
			SaveDebugImage("CG_No_Challenge", True, True, "_P1 ")
