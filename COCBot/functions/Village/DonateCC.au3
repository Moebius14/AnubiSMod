		...
		If $abDonateQueueOnly[$i] Then
			...
			For $j = 0 To 10
				...
				If _ColorCheck(_GetPixelColor($xQueue, 158 + $g_iMidOffsetY, True), Hex(0xD7AFA9, 6), 20) Then ; Pink background found at $xQueue
					ExitLoop
				ElseIf _ColorCheck(_GetPixelColor($xQueue, 166 + $g_iMidOffsetY, True), Hex(0xD0D0C8, 6), 20) Then ; Gray background _ColorCheck(_GetPixelColor(829, 166 + $g_iMidOffsetY, True), Hex(0xD0D0C8, 6), 20)
					SetLog("2nd army is not prepared. Donate whatever exists in 1st army!!")
					$abDonateQueueOnly[$i] = False
					ContinueLoop 2
				EndIf
...
Func DonateCC($bCheckForNewMsg = False)
	...
	While 1
		...
		If IsArray($Scroll) And _ColorCheck(_GetPixelColor(301, 85, True), Hex(0x95CD0E, 6), 20) Then ; a second pixel for the green
			$bDonate = True
	...
	Local $iTimer
	Local $sSearchArea, $aiSearchArray[4] = [200, 90, 300, 680], $aiSearchArrayBackUp = $aiSearchArray
...
Func DonateTroopType(Const $iTroopIndex, $Quant = 0, Const $bDonateQueueOnly = False, Const $bDonateAll = False)
	...
	Local $g_iDonTroopsLimitClan = getOcrAndCapture("coc-t-lim", 432, $g_iDonationWindowY + 15, 28, 14, True)
...
Func DetectSlotTroop(Const $iTroopIndex)
	...
	For $Slot = 0 To 6
		Local $x = 343 + (68 * $Slot)
		Local $y = $g_iDonationWindowY + 37 ;280
		Local $x1 = $x + 75
		Local $y1 = $y + 59
