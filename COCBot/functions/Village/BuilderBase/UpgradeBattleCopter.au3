...
Func chkUpgradeBattleCopter()
	$g_bBattleCopterUpgrade = (GUICtrlRead($g_hChkBattleCopterUpgrade) = $GUI_CHECKED) ? 1 : 0
	If GUICtrlRead($g_hChkBattleCopterUpgrade) = $GUI_CHECKED Then
		If $g_bNotifyTGEnable = True Then
			GUICtrlSetState($g_hChkNotifyUpgradeBC, $GUI_ENABLE)
		Else
	...
	Else
		GUICtrlSetState($g_hChkNotifyUpgradeBC, $GUI_DISABLE)
	EndIf
EndFunc   ;==>chkUpgradeBattleCopter
...
Func LocateBattleCopter()
	...
	CollectBuilderBase(False, False, True, True, True)
