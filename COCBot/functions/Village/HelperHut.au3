...
Func AppBuilder()
	...
	If IsArray($aConfirmButton) And UBound($aConfirmButton) = 2 Then
		...
		$ActionForModLog = "Builder's Apprentice assigned"
		If $g_iTxtCurrentVillageName <> "" Then
			GUICtrlSetData($g_hTxtModLog, @CRLF & _NowTime() & " [" & $g_iTxtCurrentVillageName & "] " & $ActionForModLog & " to the " & ($g_bChkAppBuilder = 1 ? "longest" : "shortest") & " upgrade", 1)
		Else
			GUICtrlSetData($g_hTxtModLog, @CRLF & _NowTime() & " [" & $g_sProfileCurrentName & "] " & $ActionForModLog & " to the " & ($g_bChkAppBuilder = 1 ? "longest" : "shortest") & " upgrade", 1)
		EndIf
		_FileWriteLog($g_sProfileLogsPath & "\ModLog.log", " [" & $g_sProfileCurrentName & "] " & $ActionForModLog & " to the " & ($g_bChkAppBuilder = 1 ? "longest" : "shortest") & " upgrade")
...
Func LabAssistant()
	...
	If IsArray($aConfirmButton) And UBound($aConfirmButton) = 2 Then
		...
		$ActionForModLog = "Lab Assistant assigned"
		If $g_iTxtCurrentVillageName <> "" Then
			GUICtrlSetData($g_hTxtModLog, @CRLF & _NowTime() & " [" & $g_iTxtCurrentVillageName & "] " & $ActionForModLog & " to the " & ($g_bChkLabAssistant = 1 ? "longest" : "shortest") & " upgrade", 1)
		Else
			GUICtrlSetData($g_hTxtModLog, @CRLF & _NowTime() & " [" & $g_sProfileCurrentName & "] " & $ActionForModLog & " to the " & ($g_bChkLabAssistant = 1 ? "longest" : "shortest") & " upgrade", 1)
		EndIf
		_FileWriteLog($g_sProfileLogsPath & "\ModLog.log", " [" & $g_sProfileCurrentName & "] " & $ActionForModLog & " to the " & ($g_bChkLabAssistant = 1 ? "longest" : "shortest") & " upgrade")
		If _Sleep(Random(2000, 2500, 1)) Then Return
		WaitForClanMessage("BuildersApprenticeMid")
		Local $aiJobinProgress = decodeSingleCoord(FindImageInPlace2("JobInProgress", $ImgJobInProgress, 390, 220 + $g_iMidOffsetY, 490, 280 + $g_iMidOffsetY, True))
		If IsArray($aiJobinProgress) And UBound($aiJobinProgress) = 2 Then
