...
Func SearchLimit($iSkipped, $bReturnToPickupHero = False)
	If $bReturnToPickupHero Or ($g_bSearchRestartEnable And $g_iSearchCount >= Number($g_iSearchRestartLimitPause)) Then
		SetLog("Too Much Searches ! Restart in : " & $g_iSearchRestartLimitPauseTime & " mn", $COLOR_OLIVE)
		Local $Wcount = 0
		...
		$g_iSearchCount = 0
		Return True
	ElseIf $bReturnToPickupHero Or ($g_bSearchRestartEnable And $iSkipped >= Number($g_iSearchRestartLimit)) Then
		Local $Wcount = 0
		While _CheckPixel($aSurrenderButton, $g_bCapturePixel) = False
			If _Sleep($DELAYSEARCHLIMIT) Then Return
			$Wcount += 1
