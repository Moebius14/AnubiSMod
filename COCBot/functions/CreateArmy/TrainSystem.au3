	...
	If $g_bFullArmy And $g_bCheckSpells And $bFullArmyHero And $bFullSiege Then
		If Not $bFullArmyCC And $g_bRequestTroopsEnable And ((Not $bChkUseOnlyCCMedals And $g_aiCmbCCDecisionThen = 1 And $IsTofillWithMedals) Or $bChkUseOnlyCCMedals) And $IsTofillWithMedalsPause Then
			...
			If $IsCCFilled = "Filled" Then
				...
				SetLog("Remaining CC Medals : " & $MedalsSetlog, $COLOR_DEBUG1)
				$bFullArmyCC = True
				$IsTofillWithMedals = False
				$g_bIsFullArmywithHeroesAndSpells = True
				$g_bWaitForCCTroopSpell = False ; To Not Switch if fill with medals
				If $g_bNotifyTGEnable And $g_bNotifyAlertCampFull Then PushMsg("CampFull")
				SetLog("Chief, Your Army Is Ready !", $COLOR_SUCCESS)
			ElseIf $IsCCFilled = "NoMedal" Then
				SetLog("Clan Castle Fill Failed, Not Enough Medals", $COLOR_ERROR)
				SetLog("Remaining CC Medals : " & $MedalsSetlog, $COLOR_DEBUG1)
				If _Sleep(1500) Then Return
				$bFullArmyCC = True
				$IsTofillWithMedals = False
				$g_bIsFullArmywithHeroesAndSpells = True
				If $g_bNotifyTGEnable And $g_bNotifyAlertCampFull Then PushMsg("CampFull")
				SetLog("Lets Attack, No Wait !", $COLOR_SUCCESS)
			ElseIf $IsCCFilled = "WaitForCC" Then
				SetLog("Waiting For CC Then Finish With Medals", $COLOR_FUCHSIA)
			ElseIf $IsCCFilled = False Then
...
Func IsQueueEmpty($sType = "Troops", $bSkipTabCheck = False, $removeExtraTroopsQueue = True)
	...
	If $removeExtraTroopsQueue Then ; If No troops were in Queue Return True
		If _ColorCheck(_GetPixelColor(263, 215 + $g_iMidOffsetY, True, $g_bDebugSetlogTrain ? $sType & " $removeExtraTroopsQueue:0x677CB5" : Default), Hex(0x677CB5, 6), 20) Then Return True ; Check for Blue band in empty queue
	Else
		If _ColorCheck(_GetPixelColor(773, 195 + $g_iMidOffsetY, True, $g_bDebugSetlogTrain ? $sType & " $removeExtraTroopsQueue:0xD0D0C8" : Default), Hex(0xCFCFC8, 6), 20) Then Return True ; check gray background at 1st training slot
	EndIf
	...
	If $sArmyType = "Heroes" Then
		For $i = 0 To UBound($aResult) - 1
			If StringInStr($aResult[$i][0], "Kingqueued") Then
				$aResult[$i][3] = getRemainTHero(530, 381 + $g_iMidOffsetY)
			ElseIf StringInStr($aResult[$i][0], "Queenqueued") Then
				$aResult[$i][3] = getRemainTHero(595, 381 + $g_iMidOffsetY)
			ElseIf StringInStr($aResult[$i][0], "Wardenqueued") Then
				$aResult[$i][3] = getRemainTHero(657, 381 + $g_iMidOffsetY)
			ElseIf StringInStr($aResult[$i][0], "Championqueued") Then
				$aResult[$i][3] = getRemainTHero(721, 381 + $g_iMidOffsetY)
			Else
				$aResult[$i][3] = 0
			EndIf
		Next
